(() => {
  ...
  var WATCHER_URLS = [
    "http://127.0.0.1:8742/state",
    "http://localhost:8742/state"
  ];
  var _LogbookView = class _LogbookView extends AppView {
    constructor(props) {
      super(props);
      this.acft = import_msfs_sdk2.Subject.create("--");
      this.state = import_msfs_sdk2.Subject.create("ON GROUND");
      this.alt = import_msfs_sdk2.Subject.create("--");
      this.gs = import_msfs_sdk2.Subject.create("--");
      this.ias = import_msfs_sdk2.Subject.create("--");
      this.vs = import_msfs_sdk2.Subject.create("--");
      this.g = import_msfs_sdk2.Subject.create("--");
      this.bank = import_msfs_sdk2.Subject.create("--");
      this.pitch = import_msfs_sdk2.Subject.create("--");
      this.td = import_msfs_sdk2.Subject.create("--");
      this.grade = import_msfs_sdk2.Subject.create("--");
      this.note = import_msfs_sdk2.Subject.create("Live from the aircraft.");
      this.wAlive = import_msfs_sdk2.Subject.create("WATCHER --");
      this.wState = import_msfs_sdk2.Subject.create("--");
      this.wFlight = import_msfs_sdk2.Subject.create("--");
      this.wGround = import_msfs_sdk2.Subject.create("--");
      this.wEvent = import_msfs_sdk2.Subject.create("--");
      this.wAge = import_msfs_sdk2.Subject.create("--");
      this.wTime = import_msfs_sdk2.Subject.create("--");
      this.wAliveClass = import_msfs_sdk2.Subject.create("lb-watch-live off");
      this._poll = null;
      this._wPoll = null;
      this._wBusy = false;
      this._wUrlIndex = 0;
    ...
    onOpen() {
      this.tick();
      this.pollWatcher();
      this._poll = setInterval(() => this.tick(), 1000);
      this._wPoll = setInterval(() => this.pollWatcher(), 2000);
    }
    onClose() {
      if (this._poll) {
    ...
    sv(name, unit) {
      try {
        const api = import_msfs_sdk2.SimVar || (typeof SimVar !== "undefined" ? SimVar : null);
        if (!api || !api.GetSimVarValue) return NaN;
        return api.GetSimVarValue(name, unit);
      } catch (e) {
        return NaN;
      }
    }
    ...
    pollWatcher() {
      ...
      this._wBusy = true;
      const url = WATCHER_URLS[this._wUrlIndex % WATCHER_URLS.length];
      ...
      fetch(url, opts).then((r) => {
        ...
        clearTimeout(timer);
        this._wBusy = false;
        const c = d && d.current ? d.current : {};
        const e = d && d.last_event ? d.last_event : {};
        const n = d && d.notify ? d.notify : {};
        this.wAlive.set("WATCHER LIVE  pid " + (d.pid || "--"));
        this.wAliveClass.set("lb-watch-live");
        const conn = c.connected === true ? "connected" : "disconnected";
        this.wState.set((c.state || "idle") + "  ·  " + conn);
        const fid = c.flight_id || null;
        const sortie = c.sortie_id || d.sortie_id || n.sortie_id || this._sortieId || fid;
        const liftIso = c.lift_at || d.lift_at || n.lift_at || null;
        const jump = this.nmBetween(this._lastLat, this._lastLon, c.lat, c.lon);
        const quit = c.connected === false || c.state === "idle" && !fid;
        const newSpawn = c.on_ground === true && jump >= 0.5 && this._lastFid && fid && fid !== this._lastFid;
        if (fid && fid !== this._lastFid) {
          if (!newSpawn && !quit) {
            if (!this._sortieId) this._sortieId = sortie;
          } else {
            this._sortieId = sortie || fid;
            this._liftMs = liftIso ? Date.parse(liftIso) : null;
            this._airStart = null;
            this._airborneMs = 0;
          }
        }
        if (!this._sortieId) this._sortieId = sortie;
        if (liftIso && !this._liftMs) {
          const ms = Date.parse(liftIso);
          if (Number.isFinite(ms)) this._liftMs = ms;
        }
        this._lastFid = fid;
        if (Number.isFinite(c.lat)) this._lastLat = c.lat;
        if (Number.isFinite(c.lon)) this._lastLon = c.lon;
        this.wFlight.set(this._sortieId || "(no sortie)");
        this.wGround.set(c.on_ground == null ? "--" : c.on_ground ? "ON GROUND" : "AIRBORNE");
        let et = e.type || "--";
        if (et === "flight_start" && (this._wasAirborne || this._sortieId && e.flight_id && e.flight_id !== this._sortieId)) {
          et = "reconnect";
        }
        ...
        clearTimeout(timer);
        this._wBusy = false;
        this._wUrlIndex = (this._wUrlIndex + 1) % WATCHER_URLS.length;
        this.wAlive.set("WATCHER OFF");
        this.wAliveClass.set("lb-watch-live off");
        this.wState.set("cannot reach 127.0.0.1:8742");
        this.wFlight.set("--");
        this.wGround.set("--");
        this.wEvent.set("--");
        this.wAge.set("--");
    ...
    tick() {
      const onGround = !!this.sv("SIM ON GROUND", "Bool");
      const vs = this.sv("VERTICAL SPEED", "feet per minute");
      const gs = this.sv("GROUND VELOCITY", "knots");
      const ias = this.sv("AIRSPEED INDICATED", "knots");
      const alt = this.sv("PLANE ALTITUDE", "feet");
      const g = this.sv("G FORCE", "GForce");
      const bank = this.sv("PLANE BANK DEGREES", "degrees");
      const pitch = this.sv("PLANE PITCH DEGREES", "degrees");
      let title = this.sv("TITLE", "string");
      if (!title) title = this.sv("ATC MODEL", "string");
      if (!title) title = "Aircraft";
      this.acft.set(String(title));
      this.state.set(onGround ? "ON GROUND" : "AIRBORNE");
      this.alt.set(Number.isFinite(alt) ? Math.round(alt).toLocaleString() : "--");
      this.gs.set(this.fmt(gs, 1));
      this.ias.set(this.fmt(ias, 1));
      this.vs.set(Number.isFinite(vs) ? Math.round(vs).toString() : "--");
      this.g.set(this.fmt(g, 2));
      this.bank.set(this.fmt(bank, 1));
      this.pitch.set(this.fmt(pitch, 1));
      const now = Date.now();
      if (!onGround && Number.isFinite(vs)) {
        this._wasAirborne = true;
        this._lastAirVs = vs;
        if (Math.abs(vs) >= Math.abs(this._peakVs) || now - this._peakTs > 1e4) {
          this._peakVs = vs;
          this._peakTs = now;
        }
      }
      if (!onGround) {
        if (this._airStart == null) {
          this._airStart = this._liftMs || now;
          if (!this._liftMs) this._liftMs = this._airStart;
        }
        this.wTime.set(this.fmtDur(now - this._airStart));
      } else if (this._airStart != null) {
        this._airborneMs = Math.max(0, now - this._airStart);
        this._airStart = null;
        this.wTime.set(this._airborneMs ? this.fmtDur(this._airborneMs) : "--");
      } else {
        this.wTime.set(this._airborneMs ? this.fmtDur(this._airborneMs) : this._liftMs ? this.fmtDur(now - this._liftMs) : "--");
      }
      if (onGround && this._wasAirborne) {
        // The sim's own touchdown normal velocity, which is what the logbook
        // grades on. _lastAirVs is this app's own sample at <= 1 Hz, so it can
        // be a second stale and reads the steeper part of the approach rather
        // than the contact - which is why the tablet and the logbook disagreed
        // on the same landing. Fall back to it only if the simvar is absent.
        const tdn = this.sv("PLANE TOUCHDOWN NORMAL VELOCITY", "feet per second");
    ...
    metric(label, value, unit) {
      return import_msfs_sdk2.FSComponent.buildComponent(
        "div",
        { class: "lb-metric" },
        import_msfs_sdk2.FSComponent.buildComponent("div", { class: "lb-label" }, label),
        import_msfs_sdk2.FSComponent.buildComponent("div", { class: "lb-value" }, value, unit ? import_msfs_sdk2.FSComponent.buildComponent("span", { class: "lb-unit" }, unit) : null)
      );
    }
    watchRow(label, value) {
      return import_msfs_sdk2.FSComponent.buildComponent(
        "div",
        { class: "lb-watch-row" },
        import_msfs_sdk2.FSComponent.buildComponent("span", { class: "lb-watch-k" }, label),
        import_msfs_sdk2.FSComponent.buildComponent("span", { class: "lb-watch-v" }, value)
      );
    }
    render() {
      return import_msfs_sdk2.FSComponent.buildComponent(
        "div",
        { class: "logbook-page" },
        import_msfs_sdk2.FSComponent.buildComponent(
          "div",
          { class: "lb-header" },
          import_msfs_sdk2.FSComponent.buildComponent("div", { class: "lb-title" }, "AfterFlight"),
          import_msfs_sdk2.FSComponent.buildComponent("div", { class: "lb-state" }, this.state)
        ),
        import_msfs_sdk2.FSComponent.buildComponent("div", { class: "lb-acft" }, this.acft),
        import_msfs_sdk2.FSComponent.buildComponent(
          "div",
          { class: "lb-watch" },
          import_msfs_sdk2.FSComponent.buildComponent("div", { class: this.wAliveClass }, this.wAlive),
          this.watchRow("state", this.wState),
          this.watchRow("sortie", this.wFlight),
          this.watchRow("flight time", this.wTime),
          this.watchRow("watcher ground", this.wGround),
          this.watchRow("last event", this.wEvent),
          this.watchRow("updated", this.wAge)
        ),
        import_msfs_sdk2.FSComponent.buildComponent(
          "div",
          { class: "lb-grid" },
          this.metric("Flight time", this.wTime, ""),
          this.metric("Altitude", this.alt, "ft"),
          this.metric("Groundspeed", this.gs, "kt"),
          this.metric("IAS", this.ias, "kt"),
          this.metric("Vertical speed", this.vs, "fpm"),
          this.metric("G", this.g, "g"),
          this.metric("Bank", this.bank, "deg"),
          this.metric("Pitch", this.pitch, "deg"),
          this.metric("Touchdown", this.td, "fpm")
        ),
        import_msfs_sdk2.FSComponent.buildComponent("div", { class: "lb-grade" }, this.grade),
        import_msfs_sdk2.FSComponent.buildComponent("div", { class: "lb-note" }, this.note)
      );
    }
  };
  __name(_LogbookView, "LogbookView");
  var LogbookView = _LogbookView;
  var _AfterFlight = class _AfterFlight extends App {
    constructor() {
      super(...arguments);
      this.BootMode = AppBootMode.COLD;
      this.SuspendMode = AppSuspendMode.SLEEP;
    ...
    get icon() {
      return "coui://html_ui/efb_ui/efb_apps/AfterFlight/Assets/app-icon.svg";
    }
    async install(_props) {
      Efb.loadCss("coui://html_ui/efb_ui/efb_apps/AfterFlight/AfterFlight.css");
      return Promise.resolve();
    }
    get compatibleAircraftModels() {
      return void 0;
    ...
    render() {
      return import_msfs_sdk2.FSComponent.buildComponent(LogbookView, { bus: this.bus });
    }
  };
  __name(_AfterFlight, "AfterFlight");
  var AfterFlight = _AfterFlight;
  Efb.use(AfterFlight);
})();
